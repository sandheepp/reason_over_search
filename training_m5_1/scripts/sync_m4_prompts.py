...
Examples (run from the repo root):
    ...
    python training_m5_1/scripts/sync_m4_prompts.py --mode qwen35_minimal

    # An M4.4 system-locus candidate (system + user "Question: {q}", tools=auto):
    python training_m5_1/scripts/sync_m4_prompts.py --mode qwen35_recall_port

    # List available modes:
    python training_m5_1/scripts/sync_m4_prompts.py --list
...
OUT_DIR = REPO_ROOT / "training_m5_1" / "src" / "prompts"
...
def main() -> int:
    ...
    print(f"  data.train.prompt_file: training_m5_1/src/prompts/{USER_TXT.name}")
    if system_text is not None:
        print(f"  data.train.system_prompt_file: training_m5_1/src/prompts/{SYSTEM_TXT.name}")
    else:
