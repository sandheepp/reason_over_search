...
#
# IMPORTANT: training_m5_1/nemo_rl is a SYMLINK to ../training/nemo_rl/ (the
...
#       training_m5_1/nemo_rl/.venv/ in seconds-to-minutes (no re-download).
#
...
#                       committed source under training_m5_1/nemo_rl/, no re-clone)
#                       Set to bump the pinned NeMo-RL version (also requires
#                       FORCE_RECLONE=1 + breaking the symlink).
#   FORCE_RECLONE     — wipe training_m5_1/nemo_rl/ and re-clone from upstream
#                       at NEMO_RL_REF (or v0.6.0 if NEMO_RL_REF is empty).
#                       Blocked while training_m5_1/nemo_rl is a symlink (would
...
if [[ -n "${FORCE_RECLONE}" ]]; then
    ...
    log "remember to commit training_m5_1/nemo_rl/ after a successful re-clone"
fi
