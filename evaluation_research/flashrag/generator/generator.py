...
class SGLRemoteGenerator(BaseGenerator):
    ...
    def __init__(self, config):
        ...
        base = (self.config["sgl_remote_url"] or "").strip().rstrip("/")
        if base and not base.startswith(("http://", "https://")):
            base = f"http://{base}"
        self.sgl_remote_url = f"{base}/generate"
        self.tokenizer = AutoTokenizer.from_pretrained(self.model_path, trust_remote_code=True)

    def _truncate_prompt_tail(self, text: str, max_token_len: int) -> str:
        """Keep the last max_token_len tokens (multi-turn RAG grows the prefix; tail has latest state)."""
        if max_token_len < 1:
            return text
        ids = self.tokenizer.encode(text)
        if len(ids) <= max_token_len:
            return text
        return self.tokenizer.decode(ids[-max_token_len:], skip_special_tokens=True)
    ...
    def generate(
        ...
        **params,
        ...
        # max_new_tokens: prompt + completion must fit in context. SGLang rejects when the
        # total would meet or exceed the limit (e.g. 222 + 7970 = 8192 is still an error).
        # Client tokenizer can also differ slightly from the server, so keep a small slack.
        max_ctx = int(self.config["generator_max_input_len"])
        slack = 8  # strict < max_ctx on server + tokenizer mismatch vs SGLang
        # Multi-turn research prompts can exceed max_ctx; never send that raw or SGLang 400s.
        max_prompt_tokens = max(1, max_ctx - slack)
        input_list = [self._truncate_prompt_tail(t, max_prompt_tokens) for t in input_list]
        data_to_remote["text"] = input_list

        requested = generation_params.pop("max_new_tokens", None)
        if requested is None:
            requested = generation_params.get("max_tokens")
        if requested is None:
            requested = max_ctx
        requested = int(requested)
        prompt_tokens = (
            max(len(self.tokenizer.encode(t)) for t in input_list) if input_list else 0
        )
        budget = max_ctx - prompt_tokens - slack
        if budget < 1:
            budget = 1
        curr_sampling_params["max_new_tokens"] = max(1, min(requested, budget))
        ...
        response = requests.post(self.sgl_remote_url, json=data_to_remote, timeout=(30, 1800))
        if response.status_code >= 400:
            raise RuntimeError(
                f"SGLang /generate HTTP {response.status_code}: {response.text[:2500]}"
            )
        payload = response.json()
        # SGLang may return one dict per request or a list; iterating a dict yields keys (strings).
        if isinstance(payload, dict):
            response_list = [payload]
        else:
            response_list = payload

        def _finish_matched(item):
            fr = item.get("meta_info", {}).get("finish_reason")
            if fr is None:
                return None
            if isinstance(fr, dict):
                return fr.get("matched")
            return None

        generated_text = []
        for item in response_list:
            m = _finish_matched(item)
            stops = curr_sampling_params.get("stop") or []
            if m is not None and stops and m in stops:
                suffix = m if isinstance(m, str) else str(m)
                generated_text.append(item["text"] + suffix)
            else:
                generated_text.append(item["text"])
